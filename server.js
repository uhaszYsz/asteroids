...
/** Common enemy about to fire — 1s charge telegraph for clients. */
function emitEnemyCharge(room, e) {
  if (!e || e.kind !== 'common') return;
  roomBroadcast(room, {
    t: 'ech',
    id: e.id | 0,
    ms: Math.round((ENEMY_COMMON_CHARGE * 1000) / TPS)
  });
}
...
function updateEnemies(room) {
  ...
  for (let i = room.enemies.length - 1; i >= 0; i--) {
    ...
    // Common: start 1s charge telegraph when fireCd hits the charge window.
